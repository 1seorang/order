...
export function Display() {
  ...
  function handleNumberSpritePosition(digit) {
    let response = 0;
    let digitWidth = 16;
  ...
  return (
    <>
      <Container>
        <DisplayWrapper style={{ zIndex: isDisplayOff ? "-1" : "1" }}>
          <Sign negative={isNegative.toString()} />
          <Digits>
            {isError ? (
              <>
                <Digit
                  style={{
                    backgroundPosition: `-${handleNumberSpritePosition(
                      "e"
                    )}px 0`,
                  }}
                />
...
const Sign = styled.div`
  height: 36px;
  width: 16px;
  background-image: url("/assets/minus-${(props) =>
    props.negative === "true" ? "show" : "hide"}.png");
`;
